...
const baseTokensByChain: { [chainId in ChainId]?: Token[] } = {
  ...
  [ChainId.GOAT]: [WRAPPED_NATIVE_CURRENCY[ChainId.GOAT]!, USDC_GOAT, GOATED_GOAT],
...
export async function getV3CandidatePools({
  ...
  if (top2DirectSwapPool.length == 0 && topNDirectSwaps > 0) {
    ...
    top2DirectSwapPool = _.map(
      [FeeAmount.HIGH, FeeAmount.MEDIUM, FeeAmount.LOWMED, FeeAmount.LOW, FeeAmount.LOWEST],
      (feeAmount) => {
